...
class InvestmentPlanning(Network):
    ...
    def _add_lower_level_constraints(self):
        ...
        # KKT for line capacity constraints. Bi-linear are replaced by linearized constraints
        self.constraints.line_under_1 = self.model.addConstrs((self.variables.rho_under[n][m][t] <= M * (1 - self.variables.b5[n][m][t]) * M for t in self.TIMES for m, l in self.map_n[n].items() for n in self.NODES), name = "line_under_1")
